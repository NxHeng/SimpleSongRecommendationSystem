...
def run_linear_regression_app():
    ...
    danceability = st.number_input("Danceability", min_value=0.0, max_value=1.0, format="%.3f")
    energy = st.number_input("Energy", min_value=0.0, max_value=1.0, format="%.3f")
    loudness = st.number_input("Loudness (Negative Values)", min_value=-60.0, max_value=0.0, format="%.3f")
    speechiness = st.number_input("Speechiness", min_value=0.0, max_value=1.0, format="%.4f")
    acousticness = st.number_input("Acousticness", min_value=0.0, max_value=1.0, format="%.4f")
    instrumentalness = st.number_input("Instrumentalness", min_value=0.0, format="%e")  # Allow scientific notation
    liveness = st.number_input("Liveness", min_value=0.0, max_value=1.0, format="%.3f")
    valence = st.number_input("Valence", min_value=0.0, max_value=1.0, format="%.3f")
    tempo = st.number_input("Tempo", min_value=0.0, max_value=250.0, format="%.3f")
    album_total_tracks = st.number_input("Album Total Tracks", min_value=1, step=1)
    album_popularity = st.number_input("Album Popularity", min_value=0, max_value=100, step=1)
    duration = st.number_input("Duration", min_value=0.0, max_value=999999.0, format="%.6f")
    artist_followers = st.number_input("Artist Followers", min_value=0.0)
    artist_popularity = st.number_input("Artist Popularity", min_value=0, max_value=100, step=1)
